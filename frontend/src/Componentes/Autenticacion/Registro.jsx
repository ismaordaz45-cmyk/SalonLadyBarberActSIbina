...
// 🔐 Helper para SHA-1 usando Web Crypto API (crypto.subtle)
// Se usa solo para HIBP (HaveIBeenPwned) en el check de contraseñas comprometidas
const sha1Hex = async (message) => {
  ...
  // Fallback muy básico (no-criptográfico) por si crypto.subtle no está disponible
  // En navegadores modernos no debería usarse.
  console.warn("⚠️ crypto.subtle no disponible, no se puede calcular SHA-1 de forma segura.");
  return "";
};
...
function RegistroUsuarios() {
  ...
  const checkPasswordCompromised = async (password) => {
    ...
    if (!hash) {
      // Si no se pudo calcular el hash de forma segura, no bloquear el registro
      return false;
    }
