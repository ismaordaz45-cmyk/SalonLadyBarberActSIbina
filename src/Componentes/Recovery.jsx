...
function Recovery() {
  ...
  // Paso 1: Enviar email para reset
  const handleRecoverySubmit = async (e) => {
    ...
    try {
      ...
      const errorMsg = error.response?.data?.error || "Error al solicitar recuperación.";
      setError(errorMsg);
      MySwal.fire({
        icon: "error",
        title: "Error",
  ...
  // Paso 2: Verificar código y nueva password
  const handleResetSubmit = async (e) => {
    e.preventDefault();
    setIsLoading(true);
    setError("");

    if (!formData.resetCode || formData.resetCode.length !== 6 || !formData.newPassword || formData.newPassword.length < 6) {
      MySwal.fire({
        icon: "error",
        title: "Datos inválidos",
        text: "Código de 6 dígitos y contraseña mínima de 6 caracteres.",
      });
      setIsLoading(false);
      return;
    }
  ...
  const goBack = () => {
    setStep(1);
    setFormData({ correo: "", userId: "", resetCode: "", newPassword: "" });
    setError("");
  };
  ...
  const steps = ['Solicitar Código', 'Nueva Contraseña'];
          ...
          >
            ...
            >
              ...
              <Typography variant="h3" component="h1" fontWeight="700" gutterBottom fontFamily="'Playfair Display', serif">
                {step === 1 ? "Recuperar Contraseña" : "Verificar y Cambiar"}
              </Typography>
                  ...
                  <form onSubmit={handleResetSubmit}>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                      ...
                      <Button
                        ...
                        variant="contained"
                        size="large"
                        disabled={isLoading || formData.resetCode.length !== 6 || formData.newPassword.length < 6}
                        sx={{
                          py: 2,
                          fontSize: '1rem',
                          fontWeight: 500,
